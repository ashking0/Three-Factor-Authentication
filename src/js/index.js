...
elements.register.addEventListener('click', element => {

    //1. Username and Password
    if(element.target.matches(elementStrings.nextRO)) {
    ...
    //2. RGB Pattern
    if(element.target.matches(elementStrings.icon)) {
        const color = element.target.closest(elementStrings.icon).id;
        updatePattern(color);

    } else if(element.target.matches(`${elementStrings.group}, ${elementStrings.group} *`)) {

        if(element.target.closest(elementStrings.reset)) {
            clearFields();

        } else if(element.target.closest(elementStrings.nextRT)) {
    ...
    //3. Grid
    if(element.target.matches(elementStrings.nextRTH)) {
        rLevelThree();
        clear();
...
elements.login.addEventListener('click', e => {
    ...
    //1. Password
    if(e.target.matches(elementStrings.nextLO)) {
    ...
    if(e.target.matches(elementStrings.icon)) {
        ...
        } else if(e.target.closest(elementStrings.nextLT)) {
    ...
    //3. Grid
    if(e.target.matches(elementStrings.nextLTH)) {
